      ...
      <div className="space-y-6">
        <div className="card p-6">
          ...
          <form onSubmit={saveClinic} className="space-y-4">
            <div>
              ...
              <input value={f.clinic_name} onChange={(e) => setF({...f, clinic_name: e.target.value})} className="input" placeholder="Your clinic name" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Phone</label>
              <input value={f.phone} onChange={(e) => setF({...f, phone: e.target.value})} className="input" placeholder="+91 98765 43210" />
            </div>
          ...
          <form onSubmit={saveSettings} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="time" value={f.working_hours_start} onChange={(e) => setF({...f, working_hours_start: e.target.value})} className="input" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">End time</label>
                <input type="time" value={f.working_hours_end} onChange={(e) => setF({...f, working_hours_end: e.target.value})} className="input" />
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {days.map(d => (
                  <button key={d.value} type="button" onClick={() => toggleDay(d.value)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${f.working_days.includes(d.value) ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'} border`}>
                    {d.label}
                  </button>
                ))}
              </div>
        ...
        <div className="card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-lg font-medium text-slate-900">WhatsApp Reminders</h2>
          </div>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-700">Enable automated WhatsApp appointment reminders</p>
                <p className="text-xs text-slate-500 mt-1">Reminders are sent via Make.com webhook</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={f.whatsapp_enabled} onChange={(e) => setF({...f, whatsapp_enabled: e.target.checked})} className="sr-only peer" />
                <div className="w-12 h-6 rounded-full transition-colors bg-slate-200 peer-checked:bg-blue-600 relative"><span className={`absolute w-4 h-4 bg-white rounded-full top-1 transition-transform ${f.whatsapp_enabled ? 'translate-x-6' : 'translate-x-1'}`}></span></div>
              </label>
            </div>
            {f.whatsapp_enabled && (
              <div className="space-y-3 pt-2 border-t border-slate-200">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Clinic WhatsApp number</label>
                  <input value={f.whatsapp_number} onChange={(e) => setF({...f, whatsapp_number: e.target.value})} className="input" placeholder="+91 98765 43210" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Send reminder (minutes before)</label>
                  <select value={f.reminder_before_minutes} onChange={(e) => setF({...f, reminder_before_minutes: Number(e.target.value)})} className="input">
                ...
                <div>
                  ...
                  <input value={f.webhook_url} onChange={(e) => setF({...f, webhook_url: e.target.value})} className="input" placeholder="https://hook.make.com/your-webhook-url" />
                  <p className="text-xs text-slate-500 mt-1">Create a webhook scenario in Make.com that sends WhatsApp messages</p>
                </div>
              </div>
            )}
